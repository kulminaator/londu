...
(defn compose-delete-sql
  [sql-tablename old-data]
  (let [where-match-statements (build-where-match-statements sql-tablename old-data)]
    [(str
...
(defn replay-delete-in-target [event x-tgt-db]
  (let [schema (:s event)
    ...
    (j/execute! x-tgt-db (compose-delete-sql sql-tablename old-key-values))
    ))
